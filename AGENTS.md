...
7. **Update roadmap** — Before committing, update AGENTS.md: move completed items to Done, update descriptions to reflect current state, and renumber remaining items.
...
This project uses **code-review-graph** (MCP server) — a persistent knowledge graph
of the codebase. It parses all source files into a graph (functions, classes, imports,
calls) for impact analysis, dead code detection, refactoring safety, and flow tracing.

When working on a task, use the MCP tools to:
- `query_graph` — trace callers, callees, imports, tests for any symbol
- `get_impact_radius` — check blast radius before changing a hub component
- `refactor mode=rename` — safely rename symbols across the codebase
- `refactor mode=dead_code` — find unused functions/components
- `build_or_update_graph` — rebuild after significant changes
- `list_graph_stats` / `list_communities` / `list_flows` — explore structure

Always run `build_or_update_graph` after modifying files so the graph stays current.
...
- **Before starting**: Tapping "Start Fast" opens a date/time picker (default: now). User can adjust via hour/minute scrollers + date shortcuts (Today / Yesterday / 2d ago / 3d ago). A single full-width CTA at the bottom of the sheet starts the fast; its label updates dynamically to reflect the selection (e.g. "Start Fast from Yesterday at 9:00 PM" / "Start Fast from Jul 10 at 9:00 PM"), so there is no separate "Start Now" shortcut — the picker always starts the fast at the chosen date/time. The schedule preview panel updates live as the time is adjusted.
...
| 77 | **Dead code cleanup** — Removed 13 unused files (11 components, 2 hooks) identified by code-review-graph dead code analysis: `QuantityModal`, `SearchFoodModal`, `MealForm`, `CustomKeyboard`, `WaterTracker`, `WeeklyStats`, `FastingAchievements`, `MacroProgress`, `AppHeader`, `StepperControl`, `useAnimatedCounter`, `useAnimatedPressable`. All confirmed zero-import via graph query. | Done |
...
|---|-----|------|-------------|
| 1 | ~~**Food photo drops macros & serving_size**~~ | Food logging | ~~Groq vision often returns only `name` + `calories`, omitting `protein_g`/`carbs_g`/`fat_g` (logs as `0`). `serving_size` is also dropped by the frontend in both camera and library paths (`LogMealModal.tsx:272`, `:730`).~~ **Fixed**: Edge Function prompt tightened (fields required, no 0), raw response logged, `serving_size` passed through in both paths, `incomplete_fields` flag added to response. |
| 2 | ~~**No error feedback on mutations**~~ | All screens | ~~14/15 mutation hooks have `onSuccess` but no `onError`. Fast tab, Home tab, workouts, food log, period all silently swallow errors to `console.error`. Only `SettingsPanel` and `WeightTracker` use the Toast system.~~ **Fixed**: Added `defaultOptions.mutations.onError` global handler in `_layout.tsx` + `onError` callback to all 30 mutations across 11 hook files (`useFastingSession`, `useFoodLog`, `useWeightLog`, `useWorkoutGoals`, `useWorkoutLog`, `useWorkoutGroups`, `useMyMeals`, `usePeriodLog`, `useProfile`, `useFastCheckIns`, `useWaterLog`). Every mutation now logs its failure to console with a named identifier. |
| 3 | **No loading states on Fast / Home / Period / Profile** | UI | `fast.tsx` ignores the `loading` flag from `useFastingSession`. Home tab has skeletons only for weight but not fasting/cycle/workout/macros/AI panels. Period tab destructures `loading` but never uses it. Profile tab has zero loading feedback. |
| 4 | **No empty states** | UI | `PreviousFasts` returns `null` with no CTA for new users. Period tab has no "Log your first period day" empty state. Home cycle panel silently hides. |
| 5 | **Double-tap risk on Log Meal button** | Food logging | No loading state on the "Log # items" CTA in MealBuilder — slow network allows duplicate submission. |
| 6 | **Barcode scanner no loading state** | Food logging | No spinner or "Searching..." indicator between scan detection and Edge Function result. |
| 7 | **AI Insights chat no retry on error** | Home tab | `handleAskCoach` sets a static error message but no "Retry" button — user must re-type the question. |
| 8 | **Workout AI insight silent failure** | Workouts | `fetchInsight` catch block only logs to console — insight panel just never appears with no error message. |

### Remaining Features
| # | Feature | Effort | Description |
|---|---------|--------|-------------|
| 1 | **Biometric auth** | Small | Add optional Face ID / Touch ID (or fingerprint) protection on app launch. Use `expo-local-authentication` to enroll the user's biometrics. When enabled, the app shows a lock screen on startup (or after background timeout) that requires biometric verification before revealing any data. Respects the device's enrolled biometrics — no custom passcode fallback needed. **Plan**: [features/biometric-auth/PLAN.md](features/biometric-auth/PLAN.md). |
| 2 | **Home screen widget** | Medium | Build an iOS widget (WidgetKit via `expo-dev-menu` + native module) and/or Android widget (AppWidget) that shows the current fasting state: timer countdown/elapsed, progress ring, and phase badge. Widget refreshes periodically (e.g., every 15 min on iOS, every 30 min on Android). Tapping the widget deep-links into the Fast tab. **Status**: Planned — full implementation at [widgets/PLAN.md](widgets/PLAN.md). Blocked on SDK 54→56 upgrade and App Groups entitlement verification. |
| 3 | **Apple Health sync** | Medium | Two-way sync with Apple Health (iOS) and Google Fit (Android). Write fasting sessions as "Time In Bed" (a common fasting workaround) or as AFib data. Sync weight logs (already stored locally) to Health, and pull workouts from Health into the workout log. Use `expo-health-connect` (Android) and a native HealthKit bridge (iOS). Show a sync status indicator in Profile. |
| 4 | **Fasting insights / weekly trends** | Medium | Build a dedicated insights section with fasting line/bar charts showing average fast duration per week, fasting frequency (days fasted vs days skipped), streak history graph, and eating window adherence %. Add 7d / 30d / 90d filter. Use the `WorkoutTrendsChart` pattern (`components/WorkoutTrendsChart.tsx` + `hooks/useWorkoutTrends.ts`) as a reference — same `react-native-svg` approach already in use. Note: `WeeklyStats`, `FastingAchievements` components were dead code and have been removed. |
| 5 | **Export data** | Medium | Allow users to download their data as CSV files. Options: export fasting history (all sessions with start/end/duration/status), food log (all entries with macros and dates), workout log (exercise type, reps, sets, calories). Generate CSVs on-device and share via the system share sheet (`expo-sharing`). Add an "Export" button in Profile settings. |
| 6 | **Accessibility** | Medium | Support Dynamic Type (iOS) / Font Size (Android) so system font scaling affects the app. Add proper `accessibilityLabel`, `accessibilityRole`, and `accessibilityState` to all interactive elements (buttons, toggles, chips, rings). Ensure sufficient color contrast ratios across both themes. Test with VoiceOver (iOS) and TalkBack (Android). |
| 7 | **Testing** | Medium | Set up Jest + React Native Testing Library. Unit tests for Zustand stores, integration tests for hooks (useFastingSession, useFoodLog, useProfile), E2E smoke tests (Detox/Maestro) for login→fast→log→end. |
| 8 | **App Store deployment + app.json cleanup** | Medium | Prepare the app for App Store and Google Play submission: distribution builds, screenshots, description, keywords, privacy policy, Apple review checklist. Also fix `app.json`: align Android `package` to `com.ashcdev2.fasttrack` (match iOS), remove duplicate `CAMERA` permission and unused `RECORD_AUDIO` permission. |
...
| 10 | **Social features** | Large | Add community features: share fasting achievements as images (streak, total fasts, weekly consistency) via the system share sheet. Optional "friends" system where users can follow each other's public stats (streak, total fasts, weekly avg). Lightweight challenges (e.g., "7-day streak challenge") with opt-in participation and a leaderboard. All social features must be opt-in with privacy controls. |
| 11 | **watchOS companion app** | Medium | Apple Watch app showing current fasting timer, progress ring, and phase badge. Quick actions: "Break Fast", "Log Water", "Log Mood". Uses `WatchConnectivity` to sync session state from iPhone. Complications for the watch face showing elapsed time and next milestone. |
| 12 | **Live Activity / Dynamic Island** | Medium | iOS 16+ Live Activity displaying the fasting countdown timer on the lock screen and Dynamic Island. Updates in real-time without opening the app. *Note: requires paid Apple Developer account for APNs push to update the Live Activity.* |
| 13 | **Scheduled dark/light mode** | Small | Auto-switch theme based on time of day (dark at sunset, light at sunrise) rather than only following system appearance. Useful since fasting is time-bound and users often check the app at night. Toggle in Profile: Manual / System / Scheduled (with time pickers). Note: System/Dark/Light toggle is already done (handover item #55), but the time-of-day Scheduled option is not. |
| 14 | **Social sign-in (Apple / Google)** | Medium | Add Apple Sign In and Google Sign In as authentication options alongside email/password. Previous attempt failed due to Expo Go redirect URI limitations. **Current state**: Web OAuth client ID configured in Google Cloud + Supabase Google provider enabled; `expo-auth-session` + `expo-web-browser` + `expo-crypto` installed; "Continue with Google" button on login/signup screens wired to `signInWithGoogle()` in `useAuth.ts`. **Likely fix**: Switch to a development build (not Expo Go) where custom URL schemes like `fasttrack://auth/callback` are reliably intercepted by `ASWebAuthenticationSession`. See "Google SSO — Implementation Attempt" section above for full details on all approaches tried. |

#### Previously completed (moved from Remaining)
- **Query-cache persistence perf** — Replaced manual cache persistence with `PersistQueryClientProvider` + `createAsyncStoragePersister`.
- **Photo food logging** — Already built but has known bugs (see Known Bugs #1).
- **AI fasting coach** — Already built. Only gap: multi-turn conversation history (single-turn only). Add `coach_messages` table + message list UI if wanted.
