...
| 5 | **Fasting insights / weekly trends** | Medium | **Partial** — `components/WeeklyStats.tsx`, `components/FastingAchievements.tsx`, and `hooks/useWeeklyFastingStats.ts` all exist but are never imported (dead code). Still needed: a dedicated insights section with fasting line/bar charts showing average fast duration per week, fasting frequency (days fasted vs days skipped), streak history graph, and eating window adherence %. Add 7d / 30d / 90d filter. Use the `WorkoutTrendsChart` pattern (`components/WorkoutTrendsChart.tsx` + `hooks/useWorkoutTrends.ts`) as a reference — same `react-native-svg` approach already in use. |
