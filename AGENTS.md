...
#### Already removed from Remaining (verified as completed)
- **Query-cache persistence perf** — Replaced manual `hydrateQueryCache()`/`persistCache` with `PersistQueryClientProvider` + `createAsyncStoragePersister` from `@tanstack/react-query-persist-client`. Removed ~34 lines of manual serialization, timer management, and subscribe logic.
- **Photo food logging** — Already built (handover item #49). `components/FoodCamera.tsx` + `supabase/functions/food-photo` + photo picker modal work end-to-end. **Known bug**: Groq vision often returns only `name` + `calories` and omits `protein_g`/`carbs_g`/`fat_g`, so macros log as `0` (`?? 0` fallback in `supabase/functions/food-photo/index.ts`). `serving_size` is also dropped by the frontend in both camera and library paths. **Fix**: tighten the Edge Function prompt to require macros, log the raw response, and pass `serving_size` through in `LogMealModal.tsx` (both `handlePickPhoto("library")` and the `FoodCamera` callback).
- **AI fasting coach** — Already built across handover items #48 (Home tab Q&A widget), #50 (Groq `ai-coach` Edge Function with fasting/nutrition/workout/weight/cycle context), and #76 (workout-specific insight). The only gap vs the original "full chat" spec is multi-turn conversation history — currently single-turn only. If true multi-turn is wanted later, add a `coach_messages` table + message list UI. Not blocking.
